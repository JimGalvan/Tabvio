...
from tabvio.agent.runtime import AgentRuntime, build_agent_runtime
...
class RunManager(CoreRunManager):
    ...
    async def submit_follow_up(self, run_id: UUID, task: str) -> RunRecord:
        ...
        async with self._manager_lock:
            ...
            context.execution_task = asyncio.create_task(
                self._execute(
                    context,
                    {"messages": [{"role": "user", "content": follow_up_task}]},
                ),
    ...
    def _trim_live_message_events(self, context: RunContext) -> None:
        ...
        events_to_remove = message_event_count - self.MAX_LIVE_MESSAGE_EVENT_COUNT
